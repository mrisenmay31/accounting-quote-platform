...
const QuoteCalculator: React.FC = () => {
  ...
  useEffect(() => {
    const loadConfigurations = async () => {
      try {
        ...
        // Use tenant config if available, otherwise fallback to environment variables
        const airtableConfig = {
          baseId: tenant?.airtable.pricingBaseId || import.meta.env.VITE_AIRTABLE_PRICING_BASE_ID || '',
          apiKey: tenant?.airtable.pricingApiKey || import.meta.env.VITE_AIRTABLE_PRICING_API_KEY || '',
        };

        const servicesConfig = {
          baseId: tenant?.airtable.servicesBaseId || import.meta.env.VITE_AIRTABLE_SERVICES_BASE_ID || '',
          apiKey: tenant?.airtable.servicesApiKey || import.meta.env.VITE_AIRTABLE_SERVICES_API_KEY || '',
        };
  ...
  const handleGetQuoteAndSubmit = async () => {
    ...
    try {
      ...
      // Save quote to database (only if tenant is available)
      if (tenant) {
        await saveQuote({
          tenantId: tenant.id,
          formData,
        ...
        console.log('⚠️ Local dev mode - skipping database save');
      }

      // Send quote data to Zapier webhook (use tenant webhook URL or fallback to env)
      const webhookUrl = tenant?.zapierWebhookUrl || import.meta.env.VITE_ZAPIER_WEBHOOK_URL || '';
      const webhookResult = await sendQuoteToZapierWebhook(formData, quote, webhookUrl);

      // Capture and store the Quote ID (always generated by frontend)
      if (webhookResult.quoteId) {
        localStorage.setItem('currentQuoteId', webhookResult.quoteId);
        console.log('✅ Quote ID saved to localStorage:', webhookResult.quoteId);
        console.log('Verify by running: localStorage.getItem("currentQuoteId")');
      }
